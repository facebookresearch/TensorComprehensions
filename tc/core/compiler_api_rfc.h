 ...
 *   auto profilingInfo = pExecutor->run(handle, inputs, outputs, true);
 *   // alternatively:
 *   // auto kernelTiming = pExecutor->uncheckedRun(inputs, outputs, true);
 */
namespace tc {
/// Given a TC representation as a TC + TC function name entryPoint, this
/// function compiles a new TcExecutor for the specified Backend.
/// For now, contiguous output sizes are inferred given input sizes.
/// If you need another kernel for another entryPoint or another inputs,
/// options then just compile another TcExecutor; because atm we fully JIT
/// specialize on all sizes.
...
/// For now, contiguous output sizes are inferred given input sizes.
/// If you need another kernel for another TC or another inputs or options then
