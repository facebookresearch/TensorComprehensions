...
isl::union_set modifyMappingNames(
  ...
  for (auto id : identifiers) {
    auto name = id.to_str();
    auto dim = space.find_dim_by_id(isl::dim_type::param, id);
    CHECK_LE(0, dim);
  ...
  set.foreach_set([&newSet, &identifiers, &suffix](isl::set set) {
    for (auto id : identifiers) {
      auto name = id.to_str();
      auto dim = set.find_dim_by_id(isl::dim_type::param, id);
      CHECK_LE(0, dim);
...
Scop::SyncLevel MappedScop::findBestSync(
  ...
  auto activePoints1 = activeDomainPointsInclusive(stRoot, st1);
  auto activePoints2 = activeDomainPointsInclusive(stRoot, st2);
...
std::unique_ptr<MappedScop> MappedScop::makeWithOuterBlockInnerThreadStrategy(
  ...
  // 8. Insert reduction synchronizations if necessary.
