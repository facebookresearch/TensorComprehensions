...
tc::CudaMappingOptions makeBaseCliStrategy() {
  ...
  options.genericMappingOptionsView.scheduleFusionStrategy(fs)
  ...
  options.genericMappingOptionsView.outerScheduleOptions.proto
      .set_allow_skewing(DEFAULT_ALLOW_SKEWING);
  options.genericMappingOptionsView.outerScheduleOptions.proto
      .set_positive_orthant(DEFAULT_POSITIVE_ORTHANT);
  return options;
}
...
tc::CudaMappingOptions makeCliStrategy(tc::CudaMappingOptions options) {
  ...
  options.genericMappingOptionsView.outerScheduleOptions.proto
      .set_allow_skewing(FLAGS_allow_skewing);
  options.genericMappingOptionsView.outerScheduleOptions.proto
      .set_positive_orthant(FLAGS_positive_orthant);

  if (FLAGS_fix_parameters_before_scheduling !=
      DEFAULT_FIX_PARAMETERS_BEFORE_SCHEDULING) {
    options.genericMappingOptionsView.fixParametersBeforeScheduling(
        FLAGS_fix_parameters_before_scheduling);
  }
  if (FLAGS_tile != DEFAULT_TILE) {
    options.genericMappingOptionsView.tile(FLAGS_tile);
  }
  if (FLAGS_tile_imperfectly_nested != DEFAULT_TILE_IMPERFECTLY_NESTED) {
    options.genericMappingOptionsView.tileImperfectlyNested(
        FLAGS_tile_imperfectly_nested);
  }
  ...
  if (FLAGS_unroll != DEFAULT_UNROLL_FACTOR) {
    options.genericMappingOptionsView.unroll(FLAGS_unroll);
  }
  return options;
}
